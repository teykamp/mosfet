<template>
  <div
    v-if="visibility == 'visible' || visibility == 'locked'"
    style="user-select: none; display: flex; align-items: end"
    ref="outerDiv"
  >
    <div
      style="
    ...
    <div style="position: relative; display: inline-block">
      ...
      >
        <div
          v-for="tickDiv in tickDivs"
          :style="`position: relative; width: ${tickDiv.widthPx}px; height: ${
            tickDiv.type == 'major' ? '18px' : '12px'
          }; visibility: ${tickDiv.type == 'spacer' ? 'hidden' : 'visible'}`"
          :class="{
            sliderBody: true,
      ...
      >
        <div
          style="
            position: relative;
            width: 20px;
            transform: translate(-5px, 22px) rotate(90deg);
          "
          :class="{
            sliderBody: true,
            visible: visibility == 'visible',
        ...
        <div>
          <div style="position: relative">
            <div
              style="width: 15px; transform: translate(1px, 19px) rotate(65deg)"
              :class="{
                sliderBody: true,
                visible: visibility == 'visible',
                locked: visibility == 'locked',
            ...
            <div
              style="
                width: 15px;
                transform: translate(1px, 24px) rotate(-65deg);
              "
              :class="{
                sliderBody: true,
                visible: visibility == 'visible',
      ...
      <input
        ...
        @blur="setDeviceUnselected(htmlSlider)"
...
const tickDivs: ComputedRef<TickDiv[]> = computed(() => {
  ...
  const standardSpacerWidth =
    (sliderWidthPx.value * 0.5) / (maxValue.value - minValue.value) -
    (tickWidthPx / 2) * 2;

  const firstSpacerWidthPx =
    (sliderWidthPx.value * (tickValue - minValue.value)) /
...
watch(
  [
    () => props.htmlSlider.value,
    () => props.htmlSlider.temporaryMinValue,
    () => props.htmlSlider.temporaryMaxValue,
    () => props.htmlSlider.visibility,
    () => props.htmlSlider.preciseDragging,
    () => props.htmlSlider.updated.value,
  ],
  ([
    newValue,
    newMinValue,
...
watch(
  [
    () => props.htmlSlider.value,
    () => props.htmlSlider.fromNode.value.voltage,
    () => props.htmlSlider.toNode.value.voltage,
  ],
  ([sliderValue, _, __]) => {
...
watch(
  () => props.htmlSlider.selected.value,
  () => {
    if (
      props.htmlSlider.selected.value &&
      props.htmlSlider.selectionChanged.value
    ) {
...
const onPointerDown = (event: PointerEvent) => {
  ...
  props.htmlSlider.selected.value = true;
  props.htmlSlider.checkDrag(
    { x: 0, y: 0 },
    eventInitiatesPreciseDragging(event)
  );
...
const onKeyDown = (event: KeyboardEvent) => {
  ...
  if (["Up", "ArrowUp", "Down", "ArrowDown"].includes(event.key)) {
    ...
    if (["Up", "ArrowUp"].includes(event.key)) {
      props.htmlSlider.value =
        Math.ceil(props.htmlSlider.value / sliderStepSize.value + 1) *
        sliderStepSize.value;
    } else if (["Down", "ArrowDown"].includes(event.key)) {
      props.htmlSlider.value =
        Math.floor(props.htmlSlider.value / sliderStepSize.value - 1) *
        sliderStepSize.value;
    }
  }
  ...
  if (["Vgs", "Vds", "Vsg", "Vsd"].includes(props.htmlSlider.name)) {
    const deviceIsNmos =
      props.htmlSlider.name == "Vgs" || props.htmlSlider.name == "Vds";
    const sliderIsVgs =
      props.htmlSlider.name == "Vgs" || props.htmlSlider.name == "Vsg";
    const sliderIsOnRightOfDevice =
      props.htmlSlider.transformationMatrix.isMirrored == deviceIsNmos;

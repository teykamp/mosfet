...
const selectOtherSlider = (
  sliderGroup: Named<HtmlSlider[]>,
  sliderIdxToSelect: number
) => {
...
const onKeyDown = (event: KeyboardEvent, sliderGroup: Named<HtmlSlider[]>) => {
  ...
  if (!anySlidersSelected()) {
    if (DIRECTIONS.includes(keysToDirections[event.key])) {
      if (
        Object.keys(sliderGroup.adjacencyList).includes(
          keysToDirections[event.key]
        )
      ) {
        const deviceKey =
          sliderGroup.adjacencyList[keysToDirections[event.key]];
        if (deviceKey != "") {
          ...
          if (!deviceExists) {
            console.error(
              "Could not find device by key ''" + String(deviceKey) + "'"
            );
          }
        }
      }
